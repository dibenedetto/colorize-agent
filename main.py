...
class MultilingualColorReconstructionAgent:
	def __init__(self, language="english"):
		...
		if language != "english":
			self.translator = pipeline("translation", model=f"Helsinki-NLP/opus-mt-{self.language_code}-en", device=self.device)
		else:
	...
	def _setup_language_patterns(self):
		...
		color_pattern_templates = {
			"english": [
				r"((?:\w+\s)?(?:{colors}))\s+(?:colored\s+)?((?:\w+\s*){instances})",
				r"((?:\w+\s*){instances})\s+(?:is|are|was|were|appears|appeared|looks|looked)\s+(?:\w+\s)?({colors})"
			],
			
			"latin": [
				r"((?:\w+\s)?(?:{colors}))\s+((?:\w+\s*){instances})",
				r"((?:\w+\s*){instances})\s+(?:est|sunt|erat|erant|videtur|videntur)\s+(?:\w+\s)?({colors})"
			],
			
			"french": [
				r"((?:\w+\s)?(?:{colors}))\s+((?:\w+\s*){instances})",
				r"((?:\w+\s*){instances})\s+(?:est|sont|était|étaient|paraît|paraissent|semble|semblent)\s+(?:\w+\s)?({colors})"
			],
			
			"italian": [
				r"((?:\w+\s)?(?:{colors}))\s+((?:\w+\s*){instances})",
				r"((?:\w+\s*){instances})\s+(?:è|sono|era|erano|appare|appaiono|sembra|sembrano)\s+(?:\w+\s)?({colors})"
			],
			
			"spanish": [
				r"((?:\w+\s)?(?:{colors}))\s+((?:\w+\s*){instances})",
				r"((?:\w+\s*){instances})\s+(?:es|son|era|eran|aparece|aparecen|parece|parecen)\s+(?:\w+\s)?({colors})"
			],
			
			"german": [
				r"((?:\w+\s)?(?:{colors}))\s+((?:\w+\s*){instances})",
				r"((?:\w+\s*){instances})\s+(?:ist|sind|war|waren|erscheint|erscheinen|sieht|sehen)\s+(?:\w+\s)?({colors})"
			]
		}
		...
		for template in color_pattern_templates[self.language]:
			pattern = template.format(colors=color_terms[self.language], instances="{1,3}")
...
def reconstruct_artwork_colors(image_path, text_description, language="english"):
	"""
	...
	"""
	agent = MultilingualColorReconstructionAgent(language=language)
...
if __name__ == "__main__":
	...
	image_path = "sarcofago.png"

	text_description = """
Le ali dell’aquila e degli Eroti clipeofori sono dipinte con due tipi di rosso : cremisi
e ocra, identificato dalle analisi come ematite. Le piume brevi degli Eroti erano dipinte
nelle aree scanalate di azzurro, creando un gioco di effetto chiaro scuro con i due
rossi. La pittura è realizzata con pennellate fini ed eleganti, è meticolosa e attenta ai
dettagli del rilievo scolpito.
La benda di Tellus, le corde della lira di Achille e la coda del centauro alla destra del sarcofago
erano dipinte di ocra.
I fori degli angoli degli occhi del gruppo di destra (del sarcofago) di Chirone e Achille, e quelli di
Oceanus e di Tellus erano dipinti di cremisi, mentre il contorno dell’iride dei due Eroti è
reso con il ocra; la pupilla è di colore nero.
	"""

	language = "italian"

	result, vis = reconstruct_artwork_colors(image_path, text_description, language)
